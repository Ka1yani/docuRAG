...
from app.services.documentProcessors.ocr_backends.base import OCRBackend, OCRResponse
...
PROMPTS = [
    """<image>
<|grounding|>
You are an OCR system extracting structured knowledge for a retrieval system.

Extract the document into structured markdown with the following format:

- Use headings (#, ##, ###) for hierarchy
- Keep each section semantically complete
- Extract tables in markdown format
- Extract key-value pairs clearly
- Preserve exact wording (no summarization)
